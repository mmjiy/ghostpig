	...
	var core = function(){
		...
		var copyDefaultDir = function(){
			...
			if(f.exists(widgetDir)){
				...
				//将所有widget/images复制到html/i
				fs.readdirSync(widgetDir).forEach(function(dir){
					var source = widgetDir + '/' + dir;
					if(f.isDir(source) && f.exists(source + '/i') ){
						f.mkdir(jdf.config.htmlDir+'/i');
						f.copy(source + '/i', outputdir + '/' + jdf.config.htmlDir+ '/i');
					};
				});
...
function backupProject(dirname){
	if (/branches|trunk/.test(jdf.currentDir) ) {
		...
		$.log('ghostpig backup ['+logMsg+'] to "'+ tagsDirName + '" done! ');
	}
}

var online=function(ghostpig,callback){
    var codecdn=ghostpig.config.svn.code;
    var htmlcdn=ghostpig.config.svn.html+"/"+ghostpig.config.svn.htmlonlinedir;
    var htmlDir=ghostpig.config.outputDirName+"/"+ghostpig.config.htmlDir,
        cssDir=ghostpig.config.outputDirName+"/"+ghostpig.config.cssDir,
        jsDir=ghostpig.config.outputDirName+"/"+ghostpig.config.jsDir,
        widgetDir=ghostpig.config.outputDirName+"/"+ghostpig.config.widgetDir,
        csscdn=codecdn+"/"+ghostpig.config.cssDir,
        jscdn=codecdn+"/"+ghostpig.config.jsDir,
        widgetcdn=codecdn+"/"+ghostpig.config.widgetDir;
    var username=ghostpig.config.svn.name,
        password=ghostpig.config.svn.password;
    if(codecdn && htmlcdn){
        var svn=require("./svn.js");
        var corecallback=[];
        var core=function(cc,dir,text){
            $.log("online "+text+" upload...","ghost online");
            var ret={};
            var coreb=function(cc,dir,text){
                corecallback.push(function(){
                    core(cc,dir,text);
                });
                return {core:coreb}
            }
            ret.core=coreb;
            svn.svn_save(cc,dir,username,password,function(e,result){
                if(e){
                    $.log("online "+text+" save success","ghost online");
                }
                else{
                    $.error("online "+text+" save error:"+result,"ghost online");
                }
                if(corecallback.length>0){
                    var deleteobj=corecallback.splice(0,1)[0];
                    if($.isFunction(deleteobj))deleteobj.call();
                }
            },true);
            return ret;
        }
        core(htmlcdn,htmlDir,"html").core(jscdn,jsDir,"js").core(csscdn,cssDir,"css").core(widgetcdn,widgetDir,"widget");

    }
    else{
        $.error("you have no config htmlcdn or codecdn","ghost online")
        callback();
    }
}