	...
	read:function(path,encodeing){
		if (this.exists(path)){
			try {
                ...
                encodeing=encodeing.toLowerCase();
                if(encodeing == "gbk"){
                    return iconv.decode(fs.readFileSync(path),"gbk")
                }
                else if(encodeing=="buffer"){
	...
	*/
	write:function(path,source,encodeing){
		try {
            ...
            encodeing=encodeing.toLowerCase();
            if(encodeing == "gbk"){
                var buffer=iconv.encode(source,"gbk");
			...
			if(f.isDir(source)){
				...
				if(removedAll) {
					...
					try {
						...
						console.log(e);
						console.log('jdf del dir error');

						//mac svn dir : EACCES, permission denied 'dirname'
						if(e.errno ==3){
							console.log(e.message);
						}else{
							//win7下打开当前文件夹时删除会报错,所以再次执行删除
							console.log('try again');
							f.del(source, callback); //后续代码有报错，会造成死循环
						}
					}
					*/
				}
