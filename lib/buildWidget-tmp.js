...
buildWidget.init = function(inputPath,content,type,callback){
    ...
    else if(isBuild)content = staticUrlReplace(content);
	...
	var cssComboArr = [];
	var jsComboArr = [];
    var LocalServer="http://localhost:"+jdf.config.localServerPort;
    if(!jdf.config.local && isBuild)jdf.config.cdn=LocalServer;
    else if(isOutput && jdf.config.newcdn)jdf.config.cdn=jdf.config.newcdn;
	//widget
	if (result){
		//取得html中所有注释内容
				...
				files.forEach(function(item){
					...
					var vmCompileFn = function(vmContent){
						...
						try {
							...
							$.log('ghostpig widget ' +widgetName  +' data error');
							$.log(e);
							return;
						}
