	...
	var core = function (){
		...
		fs.readdirSync(target).forEach(function(item){
			if ( item && f.excludeFiles(item) ){
				...
				if ($.is.tpl(item) || $.is.vm(item) || $.is.handlebars(item)) {
					hasTpl = true;
...
*/
widget.install = function(name, force,callback){
	var force = typeof(force) == 'undefined' ? false : force;

	var source = jdf.config.widgetServerDir +'/'+name;
	var target = f.currentDir()+'/widget/'+name,
        username=jdf.config.svn.name,
        password=jdf.config.svn.password,
        callback=(callback && callback.call)?callback:function(){};
    //如果存在svn.name svn.password 则使用svn上传
    if(username && password){
        var source = jdf.config.widgetcdn +'/'+name;
        var core=function(){
            var svn=require("./svn.js");
            console.log("jdf widget "+name+" install start...")
            svn.svn_checkout_nosvn(source,target,username,password,function(e,result){
                if(e){
                    console.log('jdf widget "'+ name + '" install done!');
                    callback.call();
                }
                else{
                    console.log('jdf error[widget install]:'+result);
                }
            });
        }
        if(f.exists(target)){
            $.askbool("Module is installed,are you sure update?",function(){
                $.log("over","widget install");
            },core);
...
widget.publish = function(name, force){
    var publishDir = jdf.config.widgetServerDir,
        username=jdf.config.svn.name,
        password=jdf.config.svn.password;
    if (widget.pathCheck(name)) {
        return;
    }
    //如果设置了svn.name 则使用svn上传 而不使用ftp
    if(username && password){
        console.log("jdf widget '"+name+"' publish start...");
        var svn=require("./svn.js");
        var local="widget/"+name;
        var tarurl=publishDir+"/"+name;
        svn.svn_save(tarurl,local,username,password,function(e,result){
            if(e){
                console.log("jdf widget '"+name+"' publish done!");
            }
            else{
                console.log("jdf widget '"+name+"' publish error:"+result);
            }
        })
    }
    //使用FTP上传
...
*/
widget.list = function(islocal){
    if(islocal === "-local"){

    }
    else if(jdf.config.svn.name && jdf.config.svn.password){
        var svn=require("./svn.js");
        var widgetcdn=jdf.config.svn.widget;
        svn.svn_list(widgetcdn,jdf.config.svn.name,jdf.config.svn.password,function(e,result){
            if(e){
                ...
                for(var x in resultarr){
                    if(resultarr[x]){
                        $.log("第"+(n++)+"个模块："+resultarr[x].replace(b,"").replace(a,""));
                    }
                }
            }
            else{
                $.error(result,"widget list")
            }
        })
    }
    else {
...
widget.create = function(name){
	...
	profile.create().then(function (error, answers) {
        ...
        if(answers.handlebars == 'y') createFilesArray.push('handlebars');
		if(answers.js == 'y') createFilesArray.push('js');
		if(answers.scss == 'y') createFilesArray.push('scss');
